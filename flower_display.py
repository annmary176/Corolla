...
<body>
    ...
    <div id="flowerView">
        <button class="primary-btn back-btn" onclick="closeFlowerView()">← Back to List</button>
    ...
    <script>
        ...
        async function openFlowerView() {
            ...
            // Get actual data from localStorage to make the flower relevant
            const data = JSON.parse(localStorage.getItem('testResults') || '{}');
            
            // Collect features for the flower
            const features = {
                reading_score: data.test1 ? data.test1.array[0] : 70,
                logic_score: 65,
                writing_score: 40,
                memory_score: 80,
                reading_time: data.test1 ? data.test1.array[1] * 1000 : 10000,
                logic_time: 12000,
                writing_time: 20000,
                memory_time: 8000
            };
            ...
            const renderFlower = (result) => {
                document.getElementById('center-score').innerText = Math.round(result.overall_avg) + "%";
                document.getElementById('flower-title').innerText = result.performance_level + " Performance Profile";
                ...
                const updatePetal = (id, val) => {
                    ...
                    p.style.background = val > 60 ? 'linear-gradient(135deg, #fca5a5, #ef4444)' : 
                                       val > 30 ? 'linear-gradient(135deg, #fbcfe8, #f472b6)' : 
                                       'linear-gradient(135deg, #c7d2fe, #6366f1)';
                };

                updatePetal('dyslexia', result.petals.dyslexia);
                updatePetal('dyscalculia', result.petals.dyscalculia);
                updatePetal('dysgraphia', result.petals.dysgraphia);
                updatePetal('adhd', result.petals.adhd);
            };
            ...
            try {
                ...
                // FALLBACK LOGIC: Estimate risk based on scores if backend is down
                const avgScore = (features.reading_score + features.logic_score + features.writing_score + features.memory_score) / 4;
                const fallbackData = {
                    overall_avg: avgScore,
                    performance_level: avgScore > 75 ? "High" : avgScore > 45 ? "Moderate" : "Lower",
                    petals: {
                        dyslexia: 100 - features.reading_score,
                        dyscalculia: 100 - features.logic_score,
                        dysgraphia: 100 - features.writing_score,
                        adhd: (100 - features.memory_score) + 10 // Mock variance
                    }
                };
                renderFlower(fallbackData);
            }
        }

        function closeFlowerView() { document.getElementById('flowerView').style.display = 'none'; }
        function logout() { sessionStorage.removeItem('parentLoggedIn'); location.reload(); }
